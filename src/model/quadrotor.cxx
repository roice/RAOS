...
QRdynamic::QRdynamic(float* pos_ref, float* pos, float* vel, float* acc, float* att, float* att_vel, float delta_t, const char* robot_name, const char* ctl_name, QRframe_t* frm, SimEnvInfo* sim_env_info, float* wind, float* wind_est_incl, float* wind_est_leso)
{
...
void QRdynamic::quadrotor_model(void)
{
    ...
    float tau_B[3] = {
        L*k*(motor_rot_speed[0]*motor_rot_speed[0] + motor_rot_speed[1]*motor_rot_speed[1] - motor_rot_speed[2]*motor_rot_speed[2] - motor_rot_speed[3]*motor_rot_speed[3]),
        L*k*(-motor_rot_speed[0]*motor_rot_speed[0] + motor_rot_speed[1]*motor_rot_speed[1] + motor_rot_speed[2]*motor_rot_speed[2] - motor_rot_speed[3]*motor_rot_speed[3]),
...
/* Step 1: Position Control */
    ...
    err_alt = applyDeadband(err_alt, 0.01); // 1 cm deadband, remove small P parameter to reduce noise near zero position
    ...
    err_yaw = applyDeadband(err_yaw, 1.0*M_PI/180.); // 1 degree deadband, remove small P parameter to reduce noise near zero position
    // PID-Controller
    //      P
    ...
    switch (QRcontroller.name) {
        case QRcontroller_PID:
            ...
            if (strcmp(robot_name, "Micro Bee") == 0) {
                ...
                if (strcmp(robot_name, "Super Bee") != 0)
                printf("Warning: quadrotor robot name \'%s\' not recognized, use the PID setting of Super Bee by default.\n", robot_name);
                ((QRcontroller_PID_Params_t*)QRcontroller.params)->P_ALT = 1.0;
                ((QRcontroller_PID_Params_t*)QRcontroller.params)->P_VEL = 400.0;
                ((QRcontroller_PID_Params_t*)QRcontroller.params)->I_VEL = 10.0;
...
void QRestimator_LESO::update(void)
{
    ...
    float u[3]; memcpy(u, QR_vel, 3*sizeof(float));
